...
AFRAME.registerComponent('ui-panel-timeline', {
	...
	init: function (){
		...
		//TO-DO - need to figure out the positioning of T- level 1 ... calculate height of last element 
		//before creating a new one then subtract that from y to create the baseposition of the next t-level-1 entity
				...
				switch(i){
					case 0: //eg. year
						...
						if(timeCounter1 != thisTimeScale1){
							...
							//test to see if this works - bbox.height
							//bbox dimensions at infinity? -- check the geometry objects
							//text?
							var lastParentElObject3D = lastParentEl.components["ui-timeline-ts-1"].el.object3D;
							var bbox = new THREE.Box3().setFromObject(lastParentElObject3D); 
							
							
							lastParentEl.flushToDOM();
							//might not need this - layout component could work? 
							// if(tl1Counter>0){
							// 	//update the baseposition height
							// 	lastParentEl.setAttribute('ui-timeline-ts-1',{
							// 		thisBasePosition: {
							// 			x:1,
							// 			y:-bbox.height - data.margin,
							// 			z:1
							// 		}
							// 	});
							// }
							...
							timescale1Children.removeAttribute('object3D');
							...
							//if child attached, recalculate height
							// timescale1Children.addEventListener('child-attached', (e)=> {
							// 	//fix - the parent should be recalculating the height
							// 	var lastHeight = this.calculateContainerHeight(lastParentEl);
							// });
	...
	//sets the height of the geometry component of the parent container
	//based on the child components 
	calculateContainerHeight: function(parentEl){
		var el = this.el;
		var containerHeight = 0;
		//traverse all child elements of this parent
		//and containerHeight += el.getGeometryHeight()
...
AFRAME.registerComponent('ui-timeline-ts-1', {
	...
	init: function() {
		...
		//give the entity some base geometry
